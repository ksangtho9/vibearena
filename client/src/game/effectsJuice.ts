...
export function aoeDetonation(
  ...
  shockwaveRing(ctx, x, y, { color, radius: 12, expand: radius * 2.6, thickness: 5 * k + 2, ttl: 0.4 });
  shockwaveRing(ctx, x, y, { color: "#ffffff", radius: 8, expand: radius * 1.7, thickness: 2, ttl: 0.28 });
  particleBurst(ctx, x, y, { count: Math.round(14 * k), color, speed: radius * 2.2, gravity: 260, ttl: 0.5 });
