...
const SHAPE_ABILITIES = `
  ...
  "ability": {                         // the PRIMARY ATTACK (its own key + cooldown).
                                       // COMPOSE it as a "behavior" from the damage toolbox:
                                       // dealMelee{damage,range} front-arc strike · dealAoe{damage,radius,knockback}
                                       // · beam{dir,length,damage,duration} · spawnHazard fire/ice/spikes/void zones
                                       // · spawnEntity clone/minion/turret/trap/orbital/wall · boomerang{damage,range}
                                       // · applyStatus burn/stun/slow/weaken · lifesteal — plus any movement/space/draw
                                       // verbs. The behavior IS the attack.
    "name": string,                    // e.g. "Scheduled Outage"
    "kind": "aoe" | "projectile",      // FALLBACK TAG only — drives the canned effect
                                       // when you author no behavior; otherwise ignored
    "element": "fire" | "ice" | "lightning" | "poison" | "shadow" | "holy" | "arcane" | "none",
  ...
  "utility": {                         // the UTILITY ability (second key, own cooldown)
    ...
    "params": {                        // include ONLY the keys for your kind:
      ...
      "stat": "speed"|"strength"|"defense",  // buff: rider only — buff-kind REQUIRES a
      "magnitude": number              //   behavior that DOES something; bare boosts are rewritten
    },
    "cooldown": number,                // seconds, 3-10
    "power": number                    // 1-10
  },
...
GO WILD with the abilities — the engine is your canvas: flip gravity, slow time, swap places, grow huge, rain hazards, summon clones. Surprising beats safe; never balance-hedge. One hard rule: an ability's MAIN effect must be a visible ACTION — a bare stat boost is not an ability (stat nudges only ride along on real actions).`;

/** Condensed engine note for small-window models: enough verb surface to
 * author bold behaviors; kind fallbacks still catch anything malformed. */
const ENGINE_LEAN = `
COMPOSE the ATTACK as a "behavior" from damage verbs (dealMelee/dealAoe/beam/spawnHazard/spawnEntity/boomerang/applyStatus/lifesteal + movement/draw verbs) — the behavior IS the attack; "kind" only names the canned fallback. Both abilities SHOULD carry "behavior" — a small program: {"duration":s,"state":{...},"handlers":{"onCast":[Action],"onTick":[Action],"onHit":[Action],"onLand":[Action]}} (weapon handlers: onEquip/onAttack/onHitTarget). Action = {"do":VERB,...args} | {"do":"if","cond":{"lhs":V,"op":"<",..,"rhs":V},"then":[..]} | {"do":"repeat","times":n,"each":[..]} | {"do":"wait","t":s} | {"do":"set","var":x,"to":V}. Sense strings: self.x/hp, opponent.x/hp, distance, rng, state.<var>.
VERBS: leap dash teleport teleportBehind launch pushRadial setGravity setTimeScale recall setScale phase reflect tint grapple{mode:"pull"|"self"} counter{duration} charge{speed,damage} spawnProjectile{..,pierce,bounce,split} spawnEntity{kind:clone|minion|trap|turret|wall|orbital|totem} spawnEffect spawnHazard beam boomerang dealAoe dealMelee heal shield applyStatus{type:burn|stun|slow|weaken|poison|root|mark} knockback pull lifesteal screenShake flash spawnText spawnParticles drawRing drawLine drawArc drawShockwave drawLightning{x2,y2} drawSlash{angle,spread} drawBurst spawnShards{count}.
Examples: meteor storm = {"handlers":{"onCast":[{"do":"repeat","times":6,"each":[{"do":"spawnProjectile","fromAbove":true,"damage":12,"element":"fire"},{"do":"wait","t":0.25}]}]}}; hook-and-slam = {"handlers":{"onCast":[{"do":"grapple","mode":"pull","damage":6},{"do":"wait","t":0.25},{"do":"dealMelee","damage":16,"range":70},{"do":"drawSlash","spread":2}]}}; ricochet volley = {"handlers":{"onCast":[{"do":"spawnProjectile","damage":9,"count":3,"spread":0.4,"bounce":2,"split":true}]}}; venom stacks = {"handlers":{"onCast":[{"do":"spawnProjectile","damage":5,"count":2}],"onHit":[{"do":"applyStatus","type":"poison","dps":4},{"do":"applyStatus","type":"root","duration":0.8}]}}. COMBINE verbs for surprises (charge through + counter stance; totem + pierce volley; mark them, then charge). Malformed programs fall back safely, so be bold — but NEVER make an ability that is only a stat boost (kind "buff" with no behavior is rejected); stat nudges ride along on real actions.`;

/** Full engine reference — big-window models only. */
const ENGINE_RICH = `
BEHAVIOR PROGRAMS — both abilities SHOULD carry "behavior": a small program YOU write that a safe interpreter runs. When present it REPLACES the kind's canned effect (kind stays as fallback), so WRITE THE BEHAVIOR THAT IS the concept — a ninja's clone actually spawns a decoy, a meteor storm actually rains from the sky. This applies to the PRIMARY ATTACK above all: don't pick "aoe or projectile" and stop — compose the attack from the toolbox (a sustained beam, a summoned turret, a trap field, a lunging dealMelee arc, a gravity-well + dealAoe, a boomerang barrage, or something never seen); the kind tag is only what fires if you author nothing. COMBINE verbs for surprising signatures (reverse gravity then slam the fall; clone swarm that detonates on contact; mark a spot, brawl, recall to it). Malformed programs degrade safely to the kind fallback, so err bold. HARD RULE: no ability may be a bare stat boost — "buff" without a behavior is rejected and rewritten; let stat changes ride actions (dash-through that leaves haste, a stomp that weakens).
  "behavior": {
  ...
  VERBS (args optional, sensible defaults):
    movement: leap{up,forward} · dash{speed,up,iframes} · teleport{behindOpponent|x,y|dx,dy} · teleportBehind{} · applyForce{fx,fy} · setVelocity{vx,vy} · launch{target,power} (pop airborne) · pushRadial{x,y,radius,force} (shockwave out) · grapple{mode:"pull"|"self",damage} (chain-hook: yank the foe to you, or zip to them) · charge{speed,damage} (committed rush that strikes on body contact)
    ...
    spawn: spawnProjectile{damage,speed,angle,count,spread,homing,element,arc,fromAbove,pierce,bounce,split} (fromAbove rains onto the foe; pierce flies through; bounce ricochets; split shatters into a fan on hit) · spawnEntity{kind:"clone"|"minion"|"trap"|"turret"|"wall"|"orbital"|"totem", hp, ttl, count, atOpponent} (clones = weak bot copies of YOU that fight with your weapon, max 2; totem = a killable post that pulses healing to you) · spawnEffect{motif,element,radius} · spawnHazard{kind:"fire"|"ice"|"spikes"|"void",x,radius,ttl} (ground zone) · beam{dir,length,damage,duration} (sustained laser) · boomerang{damage,range} (flies out + returns)
    combat: dealAoe{damage,radius,x,y,knockback,color,particles} · dealMelee{damage,range} · heal{amount} · shield{duration,coverage} · counter{duration} (brief stance: negate + riposte the next MELEE hit — reflect covers projectiles, counter covers fists) · applyStatus{type:"burn"|"stun"|"slow"|"weaken"|"poison"|"root"|"mark", duration, dps|factor} (poison STACKS its dot per application; root pins their feet, arms still free; mark amplifies the NEXT hit they take) · knockback{strength,up} · pull{strength} · lifesteal{damage,percent}
    ...
    DRAW YOUR OWN LOOK: spawnParticles{x,y,count,color,size,spread,speed,gravity,lifetime,shape:"circle"|"square"|"spark"|"star"} · drawRing{x,y,radius,expand,color,thickness,ttl} · drawLine{x,y,x2,y2,color,width,ttl} · drawArc{x,y,radius,a0,a1,color,width,ttl} · drawShockwave{radius,expand,thickness} (wobbled impact wave) · drawLightning{x,y,x2,y2} (jagged bolt, defaults caster→foe: {"do":"drawLightning"}) · drawSlash{angle,radius,spread} (crescent swipe: {"do":"drawSlash","spread":2}) · spawnShards{count,speed} (spinning debris: {"do":"spawnShards","count":10}) · drawBurst{radius} (spike-flash pop) — most verbs also take a color override. USE THESE: every ability should have a DISTINCT designed look (shape + motion, not just rings).
...
EXOTIC WEAPONS — when the concept's weapon is NOT a held object (a body-part emitter, pure energy, floating constructs, unarmed fists, an aura), do NOT pick the nearest held form and stop there. AUTHOR the weapon:
  ...
  Behavior examples:
    ...
    hook, reel, gut: {"handlers":{"onCast":[{"do":"grapple","mode":"pull","damage":5},{"do":"drawLightning"},{"do":"wait","t":0.25},{"do":"dealMelee","damage":15,"range":75},{"do":"drawSlash","spread":2.2}]}}
    matador riposte: {"handlers":{"onCast":[{"do":"counter","duration":1.4},{"do":"tint","target":"self","color":"#ffd75e","duration":1.4},{"do":"spawnText","text":"TRY ME"}]}}
    plague dart ritual: {"state":{"stacks":0},"duration":6,"handlers":{"onCast":[{"do":"spawnProjectile","damage":6,"count":2,"homing":true,"element":"poison"}],"onHit":[{"do":"set","var":"stacks","to":{"op":"+","a":"state.stacks","b":1}},{"do":"applyStatus","type":"poison","dps":4},{"do":"if","cond":{"lhs":"state.stacks","op":">=","rhs":3},"then":[{"do":"applyStatus","type":"root","duration":1},{"do":"spawnText","text":"SEPSIS"}]}]}}
    bullhorn stampede: {"handlers":{"onCast":[{"do":"applyStatus","type":"mark","factor":1.8},{"do":"charge","speed":26,"damage":18},{"do":"screenShake","intensity":6,"duration":0.25}]}}
    totem warden: {"handlers":{"onCast":[{"do":"spawnEntity","kind":"totem","ttl":8},{"do":"spawnProjectile","damage":8,"count":4,"spread":0.6,"pierce":true}]}}
...
{"name":"Foreman Voltage","appearance":{"color":"#4a6fa5","accessories":["hard hat","tool belt"],"height":1.15,"gear":[{"kind":"armor"}]},"weapon":{"form":"warhammer","type":"melee","name":"Storm Maul","size":"large","curve":0,"spikes":2,"doubleEnded":false,"element":"lightning","parts":{"head":{"type":"hammer","size":0.9,"spikes":2},"haft":{"length":0.7,"wrapped":true},"pommel":"ring","adornments":["runes"],"material":"steel"},"properties":[{"kind":"knockback","magnitude":6},{"kind":"stagger","magnitude":5}],"range":4,"damage":8},"ability":{"name":"Scheduled Outage","kind":"aoe","element":"lightning","motif":"nova","params":{"radius":8},"cooldown":8,"power":8},"utility":{"name":"Scheduled Demolition","kind":"dash","element":"lightning","motif":"burst","params":{"distance":6},"behavior":{"handlers":{"onCast":[{"do":"setGravity","target":"opponent","scale":-1.3,"duration":1},{"do":"drawLightning"},{"do":"wait","t":0.9},{"do":"setGravity","target":"opponent","scale":2.6,"duration":0.6},{"do":"dealAoe","damage":14,"radius":100,"x":"opponent.x","color":"#9be8ff"},{"do":"screenShake","intensity":8,"duration":0.3}]}},"cooldown":9,"power":6},"stats":{"hp":7,"speed":3,"strength":9,"defense":5},"flavor":"Every demolition is on time, under budget, and slightly smited."}`;
...
{"name":"Kagerou","appearance":{"color":"#d94f30","accessories":["straw hat"],"height":1.05},"weapon":{"form":"sword","type":"melee","name":"Dawnlight Katana","size":"medium","curve":0.3,"spikes":0,"doubleEnded":false,"element":"holy","parts":{"blade":{"profile":"katana","length":0.8,"width":0.25,"edges":1,"count":1,"fuller":true,"tip":"tanto"},"haft":{"length":0.35,"wrapped":true},"guard":"disc","pommel":"none","adornments":["ribbon"],"material":"steel"},"properties":[{"kind":"bleed","magnitude":5},{"kind":"attackSpeed","magnitude":6}],"range":5,"damage":7},"ability":{"name":"Crescent Wave","kind":"projectile","element":"holy","motif":"slash","params":{"count":1,"homing":false},"cooldown":5,"power":7},"utility":{"name":"Iaijutsu Step","kind":"dash","element":"holy","motif":"slash","params":{"distance":8,"iframes":0.3},"behavior":{"handlers":{"onCast":[{"do":"dash","speed":27,"iframes":0.3},{"do":"drawSlash","spread":2.2},{"do":"dealMelee","damage":12,"range":70},{"do":"setTimeScale","target":"self","scale":1.2,"duration":1.5}]}},"cooldown":6,"power":7},"stats":{"hp":5,"speed":8,"strength":6,"defense":3},"flavor":"He has drawn his blade twice; both dawns are remembered."}`;
